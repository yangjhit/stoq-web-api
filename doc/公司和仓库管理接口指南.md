...
Stoq API提供了完整的公司和仓库管理系统,具有以下特性:

- ✅ 用户创建公司(自动成为ADMIN)
- ✅ 公司成员管理(ADMIN/MANAGER/MEMBER)
- ✅ 在公司中创建仓库(Stoq)
- ✅ 仓库详细信息管理
- ✅ 基于角色的权限控制
- ✅ 公司名称和仓库名称唯一性验证
...
|------|------|------|
| POST | `/stoqs` | 创建仓库 |
| GET | `/stoqs/company/{companyId}` | 获取公司的所有仓库 |
...
{
  "status": 400,
  "message": "Validation failed: Company name must be between 2-200 characters",
...
```bash
curl -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  http://localhost:8080/companies/my
```
...
```bash
curl -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  http://localhost:8080/stoqs/company/1
```
...
### 完整公司和仓库管理流程: 创建公司 → 创建仓库 → 查询 → 更新 → 删除
...
**详细测试报告:** 请查看 `测试报告-公司和仓库管理.md`

**测试脚本:** `test-company-stoq.sh`
